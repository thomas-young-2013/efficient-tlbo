...
class TOPO(BaseFacade):
    ...
    def predict_source_surrogate_cv(self, X, y):
        k_fold_num = 5
        _mu = list()
        ...
        for train_idx, val_idx in kf.split(X):
            idxs.extend(list(val_idx))
            X_train, X_val, y_train, y_val = X[train_idx,:], X[val_idx,:], y[train_idx], y[val_idx]
            pred_y = self.batch_predict(X_train)
            # Learn the weights of source problems.
            status, x = self.learn_source_weights(np.mat(pred_y), np.mat(y_train).T)
            if status:
                mu = np.mat(pred_y) * np.mat(x).T
            else:
                print('solver failed, and use the old weight.')
                mu = np.mat(pred_y) * np.mat(self.w[:self.K]).T
            _mu.extend(list(mu))
        assert (np.array(idxs) == np.arange(X.shape[0])).all()
        return np.asarray(_mu), None
    ...
    def train(self, X: np.ndarray, y: np.array):
        ...
        if instance_num >= self.min_num_y:
            _s_pred_y, _ = self.predict_source_surrogate_cv(X, y)
            _t_pred_y, _ = self.predict_target_surrogate_cv(X, y)
            _pred_y = np.c_[_t_pred_y.reshape((-1, 1)), _t_pred_y.reshape((-1, 1))]
