...
class SMBO_SEARCH_SPACE_Enlarge_ONLINE(SMBO_SEARCH_SPACE_Enlarge):
    def __init__(self,
                 objective_function,
                 *args, **kwargs):
    ...
    def check_space(self, X_candidate):
        print("Reduced: %.2f/%.2f, Rate: %.2f" % (
            len(X_candidate), len(self.configuration_list), len(X_candidate) / len(self.configuration_list)))
        if len(X_candidate) != len(self.configuration_list):
            self.reduce_cnt += 1
