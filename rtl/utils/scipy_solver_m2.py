...
def scipy_solve(A, b, lbd, loss_type, debug=False):
    n, m = A.shape
    ...
    def f(x):
        w = np.mat(x).T
        loss = Loss_func(b, A*w, loss_type) + lbd * np.linalg.norm(w, 2)
        return loss

    def f_der(x):
        der = Loss_der(b, A, x, loss_type) + lbd * 2 * x
        return der
    ...
    if debug:
        loss = f(res.x)
        l2_reg = lbd * np.linalg.norm(res.x, 2)
        print(loss-l2_reg, l2_reg, loss)
    return res.x, status
